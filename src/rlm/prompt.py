    ...
    *,
    ...
    if allow_recursion:
        parts.extend(
            [
                "",
                "The `rlm` module is pre-imported. `await rlm.run('sub-task')` spawns a recursive sub-agent and returns an `RLMResult` with `.answer` (string), `.usage`, `.turns`, and `.session_dir`.",
